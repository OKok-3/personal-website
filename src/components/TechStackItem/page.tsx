...
export default function TechStackItem({ title, skills }: TechStackItemProps) {
    return (
        <motion.div className={styles.container} layout="position"variants={fadeIn({ duration: 0.8 })}>
            <h3 className={`${poppins.className} ${styles.title}`}>{title}</h3>
            <motion.div className={styles.skillsContainer} variants={staggerChildren({ staggerChildren: 0.05 })}>
            {skills.map((skill) => (
                <motion.div key={skill} className={styles.logoContainer} variants={fadeIn({ duration: 0.5 })}>
