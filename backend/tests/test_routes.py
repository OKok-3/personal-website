...
class TestAuthRoutes:
    ...
    def clear_database(self, db: SQLAlchemy):
        """Set up and tear down the database for each test."""
        db.drop_all()
        db.create_all()

    @classmethod
    def create_user(self, client: FlaskClient, username: str = "testuser", password: str = "Testuser123!"):
        """Create a user."""
    ...
    def test_register_user(self, client: FlaskClient):
        ...
        response = self.create_user(client)
    ...
    def test_register_user_missing_username(self, client: FlaskClient):
        ...
        response = self.create_user(client, username=None)
